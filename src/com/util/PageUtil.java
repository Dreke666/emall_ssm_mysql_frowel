...
public class PageUtil {
	...
	public static String getPageHtml(HttpServletRequest request, long total, int page, int size) throws UnsupportedEncodingException{
		...
		// pre
		if (page <= 1) { // 如果已经是第一页, pre按钮disabled
//			pageBuilder.append("<a title='已是Top'>Top</a>");
//			pageBuilder.append("<a title='已是Top'>Pre</a>");
		}else{
		...
		// next
		if (page >= pages) { // 如果已经是最后一页, pre按钮disabled
//			pageBuilder.append("<a title='已是Last'>Next</a>");
//			pageBuilder.append("<a title='已是Last'>Last</a>");
		}else{
			pageBuilder.append("<a href='").append(url).append("?").append("page=").append(page+1)
